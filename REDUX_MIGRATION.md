...
- **7 `.js` files are left, and the mechanical work is done** — down from 114. The
  follow-on this thread asked for was scoped and run one cluster per PR: #551 deleted the
  20 dead files, #552 converted the `Utilities/` leaves, #553 the `Props` barrel with
  ...
  What remains was never in the queue.
  **`Styles/Variables/{dimensions,fonts,animations,zIndexes}.js` cannot be converted**:
  `postcss.config.js` `require.resolve()`s those four paths and requires them through
  `require-nocache` to feed `postcss-simple-vars`, and Node cannot require `.ts` — changing
  that is a build decision, not a conversion. And **the jQuery tail** —
  `Utilities/createAjaxRequest`, `Utilities/requestAction`, `Diag/ConsoleApi` — wants its
  own assessment: either `@types/jquery` and convert as-is, or port both onto `fetchJson`
  and drop the dependency, which ripples through `typings/AppError.ts` (the
  `status`/`responseJSON` shape is jQuery's), `useOAuth`, `useCaptcha` and everything that
  renders an `AppError`. #559 deleted `frontend/jsconfig.json`, which is as far as the
  config cleanup goes: the `.js` block in `eslint.config.js` **stays permanently**, because
  it also lints `babel.config.js`, `postcss.config.js` and the two files under
  `frontend/build/`, none of which will ever be TypeScript. `.js` likewise stays in
  webpack's `resolve.extensions` for as long as the four `Styles/Variables` files do.
