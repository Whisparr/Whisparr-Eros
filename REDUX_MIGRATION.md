...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **184** of 1,219 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **8,208** across 93 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **10** |
| Remaining `*Connector` files | 66 | **53** |
| Files touching React Query | 35 | **66** |
| zustand stores | 0 (not installed) | **installed, 25 files** |
...
| **Performer** *(hybrid)* — 20 redux files. Details, scenes tab, add flow, edit modal. **Index options done, #504; bulk editing and both delete paths done, #505; edit modal and the slice done, #506; details done, #507. 3 files left, 2 of them gated on Phase E.** | ~~Index options → `performerIndexOptionsStore`~~; ~~filter modal `sectionItems`~~; ~~select footer~~; ~~tags modal~~; ~~both Delete modals~~; ~~edit modal~~; ~~details → `performerScenesOptionsStore`~~; ~~`performerActions` (675 loc), `performerScenesActions`, `EditPerformerModalContentConnector.js`, `createAllPerformersSelector.ts`, `createPerformerClientSideCollectionItemsSelector.js`, `createPerformerSelector.js`, `PerformersAppState.ts`~~. Still to go: `addPerformerActions` (add flow). `PerformerDetails` and `SceneRow` keep `react-redux` only for `createUISettingsSelector`, which is Phase E. | — |
...
Performer and Studio); Settings holds 37 of the rest. #499 took the Scene one, the first of
the twelve to go. #500 did not move the number — Studio's connector is `DeleteStudioModalConnector.js`,
which belongs to the editing half; #501 took it, and found it had no importers at all.
...
Collection and `EditPerformerModalContentConnector`. Once dimensions live in zustand, a
