...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **194** of 1,227 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **8,679** across 98 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **12** |
| Remaining `*Connector` files | 66 | **54** |
| Files touching React Query | 35 | **66** |
| zustand stores | 0 (not installed) | **installed, 23 files** |
...
| **Performer** *(hybrid)* — 20 redux files. Details, scenes tab, add flow, edit modal. **Index options done, #504. 13 files left.** | ~~Index options → `performerIndexOptionsStore`~~; ~~filter modal `sectionItems`~~. `performerActions` 675 → 210 loc, keeping bulk editing, both delete paths, the monitor toggle and the edit modal's `setPerformerValue`. Still to go: `performerScenesActions`, `addPerformerActions`, `EditPerformerModalContentConnector.js` | — |
