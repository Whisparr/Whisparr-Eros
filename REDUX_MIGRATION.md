...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **205** of 1,227 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **9,294** across 99 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **13** |
| Remaining `*Connector` files | 66 | **54** |
| Files touching React Query | 35 | **66** |
| zustand stores | 0 (not installed) | **installed, 21 files** |
...
| **Studio** *(hybrid)* — 20 redux files. Same shape as Performer. **Index options done, #500; bulk editing done, #501; edit modal and the slice done, #502. 6 files left, 2 of them gated on Phase E.** | ~~Index options → `studioIndexOptionsStore`~~; ~~filter modal `sectionItems`~~; ~~select footer~~; ~~both Delete modals~~; ~~edit modal~~; ~~`studioActions` (509 loc), `studioMoviesActions`, `DeleteStudioModalConnector.js`, `createAllStudiosSelector.ts`, `createStudioSelector.js`~~. Details (`studioScenesActions`) and the add flow are what remain. | — |
