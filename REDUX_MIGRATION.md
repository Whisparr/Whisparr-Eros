...
**Status: Phases A, B and C complete. Phase D all but done — Movie is the only domain with work left that is not gated on Phase E.** See §11 for the running log.
...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **155** of 1,208 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **5,900** across 82 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **3** |
| Remaining `*Connector` files | 66 | **44** |
| Files touching React Query | 35 | **70** |
| zustand stores | 0 (not installed) | **installed, 34 files** |
...
| --- | --- | --- |
| **Movie** *(hybrid)* — do first, set the template. 29 redux files, 2 connectors. **Index options done, #496; movie editing done, #497. 11 files left, and only two are not gated on Phase E: `MovieIndex` holds a `useDispatch` it never dispatches with, and `MovieIndexSearchMenuItem` still reads the empty `movies` slice through `createMovieClientSideCollectionItemsSelector`. Retiring `movieActions` with them is the last of Phase D.** | ~~Index options → `movieIndexOptionsStore`~~; ~~select footer~~; ~~Delete modal~~; ~~filter modal `sectionItems`~~. `movieActions` keeps only `toggleMovieMonitored` (Collection) and `bulkMonitorMovie` (Performer/Studio). Retires ~~`movieIndexActions`~~; `movieTitlesActions` was already dead. | [Sonarr/Sonarr@0521a6c3](https://github.com/Sonarr/Sonarr/commit/0521a6c3) (91 files) |
...
and the app shell. **Phase D is all but done** — Scene, Performer, Studio, Collection,
interactive import, interactive search, movie files/credits and add/import movie are done;
Movie's own remainder is the last of it. It is a step up again: a domain index page sits on top of all
...
**Next: Movie's remainder, and then Phase E.** Two files and the `movies` slice, per the
Phase D table above.
...
- **Retired slices warn on upgrade** — `redux-localstorage` persists whatever the slice
  ...
  observed on every page load during that PR's verification.
