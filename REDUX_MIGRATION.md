...
**Status: complete. Phases A through F are done; there is no Redux left in the frontend.** The follow-on that came after it — the 114 `.js` files that were never in scope here — is finished too: no `.js` is left under `frontend/src`. See §11 for the running log.
...
- ~~Re-home the Sentry middleware — `createSentryMiddleware.js` is a redux middleware and
  ...
  `captureException`, both `@sentry/*` packages and the (already dead) piwik loader.
  Analytics as a feature stays and will get a replacement later, so the setting,
  `NzbDrone.Core.Analytics` and the `analytics` / `userHash` fields in `initialize.json`
  are untouched; nothing in the frontend reads those two fields today, which leaves a
  clean seam for whatever comes next.
...
handlers go with it, and `Store/Actions/index.js` loses its final slice.~~ Split three
ways, as Sonarr shipped it. ~~**Delay profiles first**, the one section of the three that
nothing outside Settings writes.~~ **Done, #544** — #518's shape plus the one thing no
section before it had, a reorder that is a server call. See *A reorder is a route, a query
string and the whole list back* below.

~~**Next: section 11b** — download client options, the single-object form.~~ **Done, #545**,
and it is the plainest conversion left: one endpoint, one `useManageSettings`, one child
under the page's toolbar. What it took beyond its own slice is the interesting part — it
was the last consumer of `createSaveHandler` *and* of `createSettingsSectionSelector`, so
...
### A reorder is a route, a query string and the whole list back

Every settings mutation the migration has written so far sends the thing it is saving: a
provider, a definition list, a bulk payload. `PUT /delayprofile/reorder/{id}` reads nothing
from its body. The profile being moved is in the route, the profile it should land after is
`?after=`, and the response is the whole reordered list. So the mutation's variables exist
only to build the path and the query string — `useDeleteMovieMutation`'s shape, where
`{ id, deleteFiles, addImportExclusion }` does the same job — and `onSuccess` writes the
response straight into `[PATH]` rather than invalidating it. That is the same bargain
`useSaveProviderSettings` strikes, and the reason no `GET /delayprofile` follows a drag.

Two pieces of the thunk had to find new homes. The **no-op guard** — a profile dropped
where it already sits sends nothing — needs the current list to compare against, so it
moved into the hook, which is where the list now lives; orders are 1-based and the drop
index is not, which is the `moveIndex + 1` in that comparison. The **drag state** (which
row is moving, where it would land) was connector state feeding the placeholder styling,
and it is local `useState` in the page now: nothing outside the fieldset ever read it.

The default profile stays out of all of this. The server gives it `order` 2147483647 and
refuses to delete it, `DelayProfileService.Reorder` skips `Id == 1` outright, and the page
renders it under the list with no drag handle. It is the one row the migration could not
treat as a list item, and `DEFAULT_DELAY_PROFILE_ID` is now named rather than the bare `1`
that appeared in four places.

### Three more things that were already dead

**`fetchDelayProfileSchema`** was a thunk and a `createFetchSchemaHandler` on
`/delayprofile/schema`. `DelayProfileController` has no such route — the resource has no
fields and no implementations, so there is nothing for a schema to describe — and nothing
in the frontend dispatched it. It has been dead since the file arrived.

**`typings/DelayProfile.ts` declared `name: string`.** `DelayProfileResource` has never
had a `Name`, no delay profile the server sends carries one, and nothing read it. The new
interface in `useDelayProfiles.ts` drops it, which is the same class of wire-type
correction as `whitelistedHardcodedSubs` in #537.

**`styles.isDraggingUp` and `styles.isDraggingDown`** were on the drag source's wrapper
`classNames` call. Neither class exists in `DelayProfileDragSource.css` — not in any
revision of it — so both resolved to `undefined` and the wrapper rendered with one class
however the drag was going. The placeholder divs above and below the row, which do have
classes, are what actually shows the drop target. TypeScript caught this on conversion;
in `.js` it was silent.

### Two creators died with one section

Download client options is a small conversion — one endpoint, one `useManageSettings`, no
new shared code — and it still removed 181 lines of `Store/`, because it was the last
caller of two generic pieces:

**`createSaveHandler`** built the PUT-the-item-back thunk every single-object settings
section used. UI, naming, media management, general, indexer options and import list
options each took their copy of that pattern to `useSaveSettings` as they converted; this
was the last one holding the file open.

**`createSettingsSectionSelector`** is the other half of the note above. #541 found that
retiring `importListOptions` collapsed the selector's `SectionsWithItemNames` generic to
`never` because `downloadClientOptions` — an item-section in the store since it was
written — had never been declared in `SettingsAppState`. Declaring it was the honest fix
and it bought exactly one PR: the connector that called the selector is the one this
section deletes, so both the declaration and the selector go now. `SettingsAppState` is
down to `downloadClients`, which is a list.

The lesson from #541 holds in the other direction too. A shared helper's last consumer is
rarely the file you expect, and it is worth grepping for it *before* writing the section
note — the row for section 11 predicted `createEnabledDownloadClientsSelector` and said
nothing about either of these.

### An empty store is not a valid store

Retiring the last slice leaves `Store/Actions/index.js` exporting `[]`, and
`createReducers` hands that to `combineReducers({})`. Redux does not treat that as an
empty-but-fine store: `getUnexpectedStateShapeWarningMessage` fires *Store does not have a
valid reducer* on every dispatch, which would have been the first console noise the
migration has produced. The store cannot go yet — `<Provider>` is still in `App.tsx` and
`FilterBuilderRowValueConnector` is still a `connect()` — so `createReducers` falls back to
a constant reducer when there are no slices, and the whole arrangement goes in F2.

That connector is worth a line of its own, because it is the last one and it never read
the store. Its `mapStateToProps` is `createSelector` over three *own props*
(`filterType`, `sectionItems`, `selectedFilterBuilderProp`), so `connect` was being used
purely to memoize a prop transformation. It is a `useMemo` waiting to happen, and F2 is
where it happens.

### What the last section actually removed

The section-11 row named four things that would go with download clients. The count was
right about the direction and wrong about the size: retiring the slice took **14 files in
`Store/Actions/Creators/`**, five selectors, `settingsActions.js`, `SettingsAppState` and
`Utilities/State/selectProviderSchema`. Not because this section used all of them, but
because it was the last section using *any* of them — the same effect #545 saw twice over,
one PR earlier and two files at a time.

Four of the deletions were already dead before this PR and are recorded here rather than
carried forward: `createStudioClientSideCollectionItemsSelector` and
`createDeepEqualSelector` had no importers at all, and neither did
`createSetClientSideCollectionFilterReducer` or `createSetProviderFieldValuesReducer`.
`OAuthInput`'s `section` prop belongs on that list too — it dispatched a failed OAuth
authorization into `settings.<section>` for "the forms still on Redux", and the last form
...
| Date | PR | What |
| --- | --- | --- |
| 2026-09-26 | #923 | **`Styles/Variables`, the last four.** `{dimensions,fonts,animations,zIndexes}` go to `.ts` the same way the palettes did in #554. `postcss.config.js` now resolves the `.ts` paths and reads `.default`, loading them through Node's built-in type stripping (Volta's 24.19 in CI, 22.18+ locally). Production `styles.css` and the chunk CSS are byte-identical to the `.js` build, apart from the chunk's source-map name. `.js` files under `frontend/src`: **0**. |
| 2026-08-28 | #560 | **The jQuery tail, and one error type.** `Utilities/createAjaxRequest` was the last jQuery in the app; it is **deleted rather than converted**, because every caller wanted something smaller than it offered. `Utilities/requestAction` and `Diag/ConsoleApi` are `.ts` on `Utilities/Fetch/fetchJson`; `getLanguageName` and the OAuth intermediate request call `fetchJson` directly; `$.param(queryParams, true)` becomes `getQueryString`, which already appended arrays the same way. `jquery` comes out of `package.json`, the webpack alias, `.tern-project` and `dependabot.yml` — **86 KB off the bundle**, and `typeof window.jQuery` is `undefined` on the running instance. With the jQuery failure shape gone there is only one error left, so **`typings/AppError` is deleted** and all ten `ApiError | AppError` unions collapse to `ApiError`. That pays for itself three times over: `getValidationFailures` in `selectSettings` loses its second implementation and delegates to the one in `useApiMutation`, `getErrorMessage` and `SpinnerErrorButton.getTestResult` lose their jQuery branches, and `EditMovieModalContent` deletes the `useMemo` that rebuilt an `ApiError` into the `{ status, responseJSON }` shape those readers wanted. **One shared file changed on purpose**: `fetchJson` sets `Content-Type: application/json` only when there is a body. It is not a CORS-safelisted request header, so sending it unconditionally preflights every cross-origin request — and the OAuth intermediate request to plex.tv is one. Same-origin callers never noticed it either way. **Three things surfaced.** (1) `createAjaxRequest` returned an `abortRequest` alongside the request and **no caller has ever used it**; the whole `XMLHttpRequest`/`aborted` apparatus was dead. (2) `useCaptcha`'s `getCaptchaCookie` attached a `done` handler and no `fail` one, so a failed request said nothing; kept, with an explicit empty `catch`, because a native promise would log an unhandled rejection where a jQuery deferred stayed quiet. (3) `getLanguageName` passed jQuery's `global: false`, which has no meaning outside jQuery's global ajax events — dropped. Verified live, and the OAuth path that the plan flagged as the risk **ran end to end against real plex.tv**: `POST /notification/action/startOAuth` → 200 with the pin url, method and headers; `POST https://plex.tv/api/v2/pins` → 201 cross-origin with no preflight failure; `POST /notification/action/continueOAuth?id=…&code=…` → 200 with the sign-in url; and `window.open` called with it. Also driven: a Webhook notification saved and tested with a bad url, both 400s rendering *Invalid Url* on the field and the Test button flashing its danger icon for three seconds; a movie edit saved with a relative path, the 400 rendering *Invalid Path* on the field (three times — the server really does return the same `PathValidator` failure three times); every `WhisparrApi` console helper (`all`, `single`, `filter`, `map`, `forEach`, `all()` over promises, `resource()`, the 404 rejection and the `delete` guard); and 22 routes with **no console errors or warnings**. **4 `.js` files left, 82 lines** — `Styles/Variables/*`, and nothing else. |
| 2026-08-28 | #559 | **The end-state cleanup.** `frontend/jsconfig.json` is deleted. It duplicated `tsconfig.json` for editors — same `src` baseUrl, same `checkJs: false`, but a stale `target: es6` and `module: commonjs` — and nothing in the build, the lint config or the test setup reads it; `tsconfig.json` already covers `./src/**/*` with `allowJs`. The tsconfig `include` also drops `./.eslintrc.js`, which has not existed since the flat config moved to the repo root. **The `.js` block in `eslint.config.js` does not go, and the plan was wrong to say it could.** Removing it was tried and reverted: it is what supplies the babel-ish parser options and the CommonJS globals, and it covers `frontend/babel.config.js`, `frontend/postcss.config.js`, `frontend/build/webpack.config.js` and `frontend/build/webpack/css-variables-loader.js` as well as `src` — 47 errors without it, mostly `'module' is not defined` and `'require' is not defined` in the four build files. Those four are Node config and will never be TypeScript, so that block is permanent regardless of what happens to the seven `.js` files under `src`. `.js` likewise stays in webpack's `resolve.extensions`. Verified: `tsc` at the three-error `node_modules` baseline, `yarn lint` clean, and a production build. The five-route smoke test claimed at the time was run against a stale bundle — the deploy went to the app's working directory rather than the one it serves from — so it verified nothing; #560's 22-route sweep runs on a tree containing this PR, and this PR changes no runtime code, so the conclusion holds. **7 `.js` files under `src`, 313 lines** — unchanged; this PR converts nothing. |
| 2026-08-28 | #558 | **The quality profile items.** `Settings/Profiles/Quality/{QualityProfileItems,QualityProfileItemGroup,QualityProfileItem,QualityProfileItemDragSource,QualityProfileItemDragPreview,QualityProfileFormatItems,QualityProfileFormatItem}` — the quality profile editor's drag-and-drop group and its custom format list, and the last mechanical cluster of the `.js` conversion. Four classes become function components; the two react-dnd files follow the `DelayProfile*` template from #544, and `QualityProfileFormatItems`' `_.debounce` class field becomes a `useMemo`'d `debounce` over a ref, which is what keeps the class's read-the-latest-props-a-second-later behaviour. `DragMoveOptions` moves out of `EditQualityProfileModalContent` and into the drag source that raises it, so the type lives with the call rather than with one of its two callers. **Checking the cluster turned up five things**, all left as they stand. (1) **Clearing a custom format's score sends `null` and the save comes back 400** — `NumberInput` reports an empty field as `null`, nothing coerces it, and the server answers `The JSON value could not be converted to System.Int32`. That response is ASP.NET's `ProblemDetails` object rather than Whisparr's validation-failure array, so `getValidationFailures` then throws `.reduce is not a function` and the modal loses its error rendering entirely — two defects, one keystroke, both confirmed live on the pre-conversion build. `QualityProfileFormatItem`'s `defaultProps = { score: 0 }`, whose comment says it is there "to handle the case score is deleted during edit", never covered it: React fills in `undefined`, not `null`. Converted with one documented cast rather than a coercion. (2) `QualityProfileItemDragSource` applied `styles.isDraggingUp` and `styles.isDraggingDown` to its wrapper and `QualityProfileItem` applied `styles.isOverCurrent` to its own — **none of the three exist in either stylesheet**, the same shape as #557's finding one file over. Removed, and `isOverCurrent` stopped being a prop, since that was its only reader. (3) The ungroup button's handler destructured a `value` off the press argument and passed it on as a second argument; `onPress` is handed a click event, so it was always `undefined`, and `handleDeleteGroupPress` takes the group id alone. (4) **Six dead props.** `dragQualityIndex` reached the drag source through a spread and nothing read it; `isInGroup`, `isDragging` and `qualityIndex` were passed to components that never declared them; `Measure` got a `whitelist` and an `includeMargin` that its typed props do not have; and `NumberInput` got a `containerClassName` that `TextInput` discards, so `.scoreContainer` has never applied. (5) The hover handler read `.y` off `getClientOffset()` unguarded, which is null once the pointer leaves the drag layer. Verified against the running instance: reorder, drag into and out of a group, group, ungroup, rename, toggle allowed, and a custom format score with its debounced reorder — each `PUT` body matching the on-screen order, the drag preview and the drop placeholder both rendering, the server's *Groups must contain multiple qualities* error rendering through `QualityProfileItems`' own `errors` list, and a clean console throughout. **7 `.js` files left, 313 lines** — and every one of them is out of scope or waiting on a decision. |
| 2026-08-28 | #557 | **The table options modal.** `Components/Table/TableOptions/{TableOptionsModal,TableOptionsModalWrapper,TableOptionsColumn,TableOptionsColumnDragSource,TableOptionsColumnDragPreview}` — the five files behind every table's **Options** button. Two classes become function components: the wrapper's whole job is one boolean, so it takes `useModalOpenState`, and the modal's five-key state becomes four `useState`s plus a `useRef` for `hasPageSize`, which the constructor froze at the section's first `pageSize` and which the wrapper's single mount keeps frozen. The drag source and preview follow the `DelayProfile*` template from #544, and the column reorder keeps its `cloneDeep` before the two splices. **`TableOptionsChangePayload.columns` is now optional**: the modal has always sent `{ pageSize }` on its own, without the columns, and the type said otherwise — the two details-year handlers that destructured `columns` and passed it straight to a setter now guard it, which is a no-op, since neither table has a page size. **Checking the cluster turned up six things**, all left as they stand. (1) `componentDidUpdate` compared the *previous* `pageSize` **prop** against the current **state** rather than against the current prop, so a page size the section rejects snaps the input back to the stored value on blur — typing `3` shows *Page size must be at least 5* and then reverts to `20`. `NumberInput` keeps its own value while focused, which is what hides it mid-edit; converted as it stands, and confirmed live in both directions. (2) `TableOptionsColumnDragSource` applied `styles.isDraggingUp` and `styles.isDraggingDown` to its wrapper, **neither of which its stylesheet has ever declared** — `classNames` dropped both `undefined`s, and the placeholder divs are what actually draw the drop position. Removed. (3) The wrapper passed `onTableOptionsChange` — plural — alongside the spread that already carried `onTableOptionChange`; nothing reads the plural. (4) `TableOptionsColumn` declared a required `index` it never reads, and the drag source passed it an `isOver` it never declared; both gone from the props and the two call sites. (5) `onColumnDragEnd` destructured an `id` off the drag item, which has no `id` and whose `id` was unused anyway — the handler takes `didDrop` alone now. (6) The drag preview rendered a `CheckInput` with **no `onChange`**, which would have thrown had it ever been changed; the preview layer is `pointer-events: none`, so it takes a no-op. Verified live: History's modal with its page size, twelve columns and ten handles; toggling **Download Client** on and seeing the header appear, then survive a reload; dragging it from position seven to position three with the reorder surviving a reload too; both validation messages (`3` → minimum, `9999` → maximum) with the snap-back above; `50` refetching as `GET /api/v3/history?page=1&pageSize=50`; the Queue's modal rendering its `optionsComponent` checkbox beside the columns; `/system/events` with `canModifyColumns={false}` showing the page size and no column list at all; and the Movies index in table view — the `VirtualTable` header-cell variant of the wrapper — toggling `showSearchAction` through the options component's own `{ tableOptions }` payload and a column through the modal's, both landing in `whisparr_movie_index_options`. The drag preview was caught mid-drag carrying the dragged column's label, tracking the cursor by `translate3d`, with one placeholder at the drop position and both gone after the drop. Console clean — no errors and no warnings. **14 `.js` files left, 1,257 lines.** |
| 2026-08-28 | #556 | **The filter builder.** `Components/Filter/Builder/{FilterBuilderRow,FilterBuilderRowValue,FilterBuilderRowValueTag,BoolFilterBuilderRowValue,ProtocolFilterBuilderRowValue,ReleaseStatusFilterBuilderRowValue,DateFilterBuilderRowValue}` and `Components/Filter/FilterModal`. Three classes become function components: `FilterBuilderRow` kept its selected prop on the instance and reassigned it beside every `onFilterChange`, which is a `useMemo` over the filter key it was always derived from; `DateFilterBuilderRowValue`'s `componentDidMount` and `componentDidUpdate` split into a mount effect and a `usePrevious`-guarded one, because only the first of the two cases ever ran on mount; and `FilterModal`'s three-key state becomes two `useState`s. **Checking the cluster turned up six things.** (1) The planned `sectionItem` typo in `FilterBuilderRowValueProps` is corrected to `sectionItems`, which lets `DefaultFilterBuilderRowValue` drop the `Omit` it needed to redeclare the real name. (2) `FilterBuilderRowValue` passed `TagInput` a `maxSuggestionsLength={100}` that **nothing reads** — not `TagInput`, not `AutoSuggestInput`, and react-autosuggest has no such prop, so it rode `otherProps` to a component that ignored it. Dropped. (3) `getValue` reads a `numberFractionDigits` off the selected prop that **no filter in the app declares**, so every number filter has always rounded to a whole number; declared optional on `FilterBuilderProp` and left as it is. (4) The tag list lookup can miss — a saved filter naming a quality profile that has since been deleted — and the row then renders the tag as the string "undefined"; kept, since coalescing changes what a stale filter shows. (5) **All ten per-section wrappers declared `{ isOpen }` and passed through five props they never named**, because `FilterMenu` types the wrapper as `React.ElementType` and `FilterModal` was untyped on the other side; they take a `FilterModalPassthroughProps` now. `PerformerIndexFilterModal` also declared a `customFilters: Filter[]` — the wrong type, and inert, since it overrides it with its own `useCustomFiltersList` fetch. (6) `FilterMenu`'s `filters` and `onFilterSelect` were spread into both modal contents, neither of which declares them; the typed boundary stops them there. `FilterBuilderRow` is generic over the section's item type and **that is where the generic dies**: the value components are shared by every section, so they take the items and the selected prop as `unknown` and hand them straight back to the section's own `optionsSelector`. `sectionItems` stays a `ReadonlyArray` from the modal down — Calendar, Collections and Interactive Search all pass readonly arrays — with one cast where the selector, declared over a plain array, is finally called. Verified live on `/movies`: seeding an empty row (key `monitored`, type `equal`, an empty bool tag list), adding `true` then `false` and getting the **or** separator between them, switching the key to `added` and watching the value swap to a date input, to `inLast` for the number-plus-unit pair, and back to `lessThan` for a date again — which is the `componentDidUpdate` conversion — the bytes round trip (`1gb` typed, `1 GiB` shown, stored as a number), the number and free-text paths, suggestions from a hook (13 quality profiles) and from `optionsSelector(sectionItems)` (16 studios), adding and removing rows, then **Save** as `ts6 probe` (`POST /customFilter` with both rows, filter applied), reopening it for edit with its tags resolved back to names, **Cancel** returning to the list rather than closing, and `DELETE /customFilter/74` removing it. Eight of the ten wrappers were opened on the running instance — the Movie, Scene, Studio, Performer, Calendar, Queue, History and Blocklist sections, covering the history-event-type, language, movie, queue-status, protocol, quality, quality-profile and default row values; Collections has no collections on this instance and Interactive Search needs a live indexer search. Console clean — no errors and no warnings. **19 `.js` files left, 1,820 lines.** |
| 2026-08-28 | #555 | **The one-off components.** `Components/{InfoLabel,Measure,withCurrentPage}`, `Movie/MovieHeadshot`, `Performer/{PerformerNameLink,PerformerStatus}`, `Scene/{NoScene,SceneStatus,SceneStudioTitleLink,SceneTitleLink}`, `Scene/Details/SceneDetailsLinks`, `Scene/MoveScene/MoveSceneModal`, `Settings/Profiles/Profiles` and `polyfills`. **Two of the sixteen this cluster listed were dead** and are deleted rather than converted: `.tsx` siblings already shadow `Components/Loading/LoadingIndicator.js` and `System/Tasks/Queued/QueuedTasks.js`, and webpack resolves `.ts`/`.tsx` ahead of `.js`. The queued-tasks copy also imports a `QueuedTaskRowConnector` that no longer exists, so it could not have built if anything had reached it — the live `.tsx` was already on `useCommands` and `QueuedTaskRow`. Nothing else in the tree is shadowed that way. The remaining fourteen follow templates already in tree — `Label.tsx` for `InfoLabel`, `MoveMovieModal.tsx` for `MoveSceneModal`, `NoStudio.tsx` for `NoScene` — and the three `PureComponent` link wrappers plus `Profiles` become function components. **Checking these for the first time turned up four things**, all recorded rather than fixed: (1) `Image['coverType']` listed four of the seven values `MediaCoverTypes` serialises, and the missing three include the `headshot` `MovieHeadshot` asks for, so the union is completed from the enum — a type-only correction, no behaviour rides on it. (2) `InfoLabel.css` has an empty `/** Kinds **/` section, so `styles[kind]` could only ever be `undefined`; the prop stays because `MovieDetails` passes it, the dead lookup goes, and the live DOM confirms the labels have only ever carried `InfoLabel-label` and a size class. (3) `InfoLabel` destructures a `title` it never renders, so the two `MovieDetails` labels that pass one have never shown it — kept as-is, and the rendered `title` attribute is still `null`. (4) `SceneIndexRow` passed `SceneStudioTitleLink` a `className` and children the component does not forward; dropped from the call site rather than wired up. `polyfills` becomes a module with a `declare global` for the non-standard `String.prototype.contains`, whose last caller went away when #553 rewrote `filterTypePredicates`; all three of its blocks are feature-detected and inert on any browser that can run this app, but dropping a polyfill is a decision rather than a conversion. `Measure` takes its ref type from `useMeasure` rather than naming `HTMLOrSVGElement`, which `react-use-measure` declares as its own unrelated type. `withCurrentPage` converts as-is even though its one consumer, `UnmappedFilesTableConnector`, ignores the `useCurrentPage` it injects. Verified live: the scene table rendering both link components with the right hrefs and titles, `SceneStatusCell` off `getSceneStatusDetails`, `SceneDetailsLinks` in the poster popover pointing at `stashdb.org/scenes/<foreignId>`, `MovieHeadshot` loading `headshot-250.jpg` on the performer index and details pages, `getPerformerStatusDetails` rendering **Active**, the `MovieDetails` info labels with a measured overview and title row, Settings → Profiles rendering all three fieldsets with 26 draggable elements — which is the `DndProvider` in `Profiles.tsx` doing its job — the queued-tasks table with its five rows, `/unmapped` through the HOC, and the **Move Files** modal reaching its `destinationRootFolder` branch with the token substituted and both buttons. Console clean on every page. **27 `.js` files left, 2,651 lines.** |
| 2026-08-28 | #554 | **The theme palettes.** `Styles/Themes/{dark,light,index}`. The two palettes change one line each — `module.exports = {` becomes `const <name> = {` plus an `export default` — and not a single colour value; `index` swaps its two `import * as` namespace imports for default imports and gains `ThemeName` and `Theme`. `Theme` is **derived from the palettes rather than written out**, so a variable added to one and not the other is a compiler error. **Except for the four that already diverge**, which are declared optional so the gap is named rather than hidden behind an index signature: `themeLightPurple` and `sceneBackgroundColor` are dark-only and are read by `PageSidebarItem.css` and five poster stylesheets, `whisparrPurple` is light-only and is read by four poster stylesheets, and `themeAlternatePurple` is light-only and is read by nothing. Confirmed on the running instance rather than inferred: a fresh load in dark leaves `--whisparrPurple` empty and one in light leaves `--themeLightPurple` and `--sceneBackgroundColor` empty, and since `ApplyTheme` only ever sets variables and never removes them, switching dark → light without a reload leaves **148** of them set — the 146 light ones plus two dark leftovers still carrying dark values. Those are palette bugs rather than conversion ones and are fixed separately. `Styles/Variables/*` stay `.js`: `postcss.config.js` requires them from Node. The default export is anonymous because `filenames/match-exported` otherwise wants the `Themes` directory renamed. Verified live: the variable set is byte-identical before and after in both palettes (146 names, same values), the Settings → UI selector saves Light, Auto and Dark with one `PUT /api/v3/config/ui` each and the page background follows, `auto` resolves through `prefers-color-scheme`, the tooltip arrow still fills from `popoverArrowBorderInverseColor` through `useThemeColor` — the one consumer that reads the palette object directly instead of a CSS variable — and fifteen pages load with a clean console. **43 `.js` files left, 3,252 lines.** |
| 2026-08-28 | #553 | **The `Props` barrel.** `Helpers/Props/index.js` is the barrel behind ~446 import sites; it and the three modules it still pulled from JavaScript (`filterBuilderValueTypes`, `filterTypePredicates`, `messageTypes`), plus `Shapes/tagShape`, `Commands/commandNames` and `Helpers/dragTypes`. Five of the seven are renames with no content change. `filterBuilderValueTypes` gains a `FilterBuilderValueType` union in the house style of `align` and `kinds`, which `Filter.ts`'s `valueType` now uses instead of a bare `string`. **`filterTypePredicates` is the exception.** Its bodies were a second copy of `getFilterTypePredicate`'s, written before that existed and without its type guards — they called `String.prototype.contains`, which is not a standard method and resolves only because `polyfills.js` defines it. Rather than write the typed implementation twice, the map is now built from `getFilterTypePredicate`, which lets `collectionFilters` and `releaseFilters` drop their `Record` casts. That is a behaviour change at the edges, and it only ever replaces a crash: a differential test over 2,640 combinations of item and filter value found the two agreeing everywhere the old map returned at all, and every difference is a case where the old one threw a `TypeError` (`contains`, `startsWith`, `endsWith` and their negations against a non-string) or had no entry to call (`inLast`, `notInLast`, `inNext`, `notInNext`). The new one returns `false` there. Neither consumer — the collection page and interactive search — has data on this instance to drive, so the differential test is the coverage for them; live, RSS Sync from the movie index posts its command and reports success through the sidebar (`commandNames`, `messageTypes`) and fifteen pages load with a clean console. **46 `.js` files left, 3,748 lines.** |
| 2026-08-28 | #552 | **The `Utilities/` leaves.** First typing cluster of the TypeScript follow-on: every file under `Utilities/` except `createAjaxRequest` and `requestAction`, which wait for the jQuery tail. 37 converted, one deleted — `Date/isAfter` has no importers and #551's sweep missed it, because every other `isAfter` in the tree is a local variable in a drag source. Most are annotation only; six bodies were rewritten where the old shape does not type (`padNumber` onto `padStart`, `firstCharToUpper` off `[].map.call`, `split` and `selectAll` off lodash, `titleCase` off `substr`, `formatAge` off parameter reassignment), and a differential test over nulls, empty strings, unicode, multi-character padding and 810 `formatAge` combinations shows every rewritten body agreeing with the original. **Four things the compiler could not see before.** `formatTimeSpan` and `formatShortTimeSpan` take a millisecond number as well as a string — `StartTime` passes `moment().diff()`. `formatDateTime`'s `dateFormat` and `timeFormat` are both optional in practice, because `getRelativeDate` passes its own optional ones straight through; `formatTime` now treats a missing format the way it treats a missing date rather than throwing on `.replace` of `undefined`. `UnmappedFilesTable` declared its selected state as `Record<number, boolean>` with a `number` `lastToggled` while the select helpers key by `number \| string`. And `parseUrl` has a return type now, so `useAddNewMovie`'s `as unknown as` cast for the query params is gone. `@types/qs` joins devDependencies rather than `parseUrl` being rewritten onto `URLSearchParams`, which parses differently. Verified on the running instance: uptime and the scheduled-task intervals render through `padNumber`/`formatTimeSpan`/`formatShortTimeSpan`, `?term=hello%20world` reaches the Add New search box through `parseUrl`, the Wanted select-all cycles 0 → 21 → 19 → 21 → 0 through `selectAll`/`areAllSelected`/`toggleSelected`, the theme names come up title-cased, a comma-separated tag entry splits into two tags, and the history details modal opens — no console errors on any of it. `formatAge` and `getQueueStatusText` have no live surface on this instance (no indexers configured, empty queue), which is what the differential test covers. **53 `.js` files left, 3,900 lines.** |
| 2026-08-28 | #551 | **The dead JavaScript.** First PR of the TypeScript follow-on, and it converts nothing: 20 files, 443 lines, no importers anywhere — `Components/Delayed`, three PropTypes shapes, two search menus, four `Scene/` helpers, and ten `Utilities/` leaves. The proof is not the greps: webpack emits a **byte-identical bundle** afterwards, same content hash, which only happens if none of the twenty contributed anything. Two live look-alikes were nearly caught in it and are called out in the PR — `Utilities/Object/hasDifferentItemsOrOrder` (used by `VirtualTable`) beside the dead `hasDifferentItems`, and `MovieIndexSearchMenuItem.tsx` beside the dead `MovieIndexSearchMenu.js`. 111 `.js` files to **91**. |
...
| 2026-08-28 | #544 | **Delay profiles.** Section 11a. `settings.delayProfiles`, `DelayProfileAppState` and `typings/DelayProfile.ts` are deleted for `Settings/Profiles/Delay/useDelayProfiles.ts` on the provider hooks; `react-redux` 22 to **19**, `Store/` 2,206 across 38 to **2,090** across 37, and all three connectors — 12 to **9**. The page is #518's shape plus the one thing no section before it had: a reorder that is a server call. `PUT /delayprofile/reorder/{id}?after=` reads nothing from its body and answers with the whole list, so the mutation's variables only build the path and the response is the cache; the thunk's no-op guard moved into the hook, where the list it compares against now lives, and the drag state became local `useState`. The default profile (order 2147483647, undeletable, skipped by the server's reorder) is now `DEFAULT_DELAY_PROFILE_ID` rather than a bare `1` in four places. Three dead things found and recorded rather than preserved: a schema thunk for an endpoint that does not exist, a `name` the resource has never had, and two class names that are in no revision of the stylesheet. Verified on the running instance: one `GET /delayprofile` on load; add (both protocols and *Only Torrent*), edit, delete, and a drag that sends `PUT /delayprofile/reorder/2?after=3` and comes back reordered with no refetch; a drop in place sends nothing; the default profile keeps its alert, its missing tags field and its missing Delete button; server validation lands on Tags; tag details reads its delay profile from the query. No console errors. |
...
- **The `.js` conversion is finished: 0 files left under `frontend/src`.** Down from 114.
  The follow-on this thread asked for ran one cluster per PR: #551 deleted the 20
  dead files, #552 converted the `Utilities/` leaves, #553 the `Props` barrel with
  `commandNames` and `dragTypes`, #554 the theme palettes, #555 the one-off components,
  #556 the filter builder, #557 the table options modal, #558 the quality profile items,
  #559 the config cleanup, #560 the jQuery tail, and #923 `Styles/Variables`. Those four
  were held back because `postcss.config.js` requires them from Node to feed
...
- **The two theme palettes are not key-identical, and three of the four odd keys are
  ...
  not; `light` defines `whisparrPurple`, which `dark` does not, and `themeAlternatePurple`,
  which nothing anywhere reads. `PageSidebarItem.css` and five poster stylesheets read the
  first two, four poster stylesheets read the third, so in the palette that lacks them the
  `var()` resolves to nothing — confirmed on the running instance in both directions.
  `ApplyTheme` compounds it: it sets variables and never removes them, so switching themes
  without a reload leaves the previous palette's extra variables behind at their old
  values. #554 named the four in the `Theme` type rather than fixing them, because picking
  replacement colours is a design call and not a conversion. Fixing it is a four-value
  change plus deleting the optional block from `Styles/Themes/index.ts`.

- **Three small pieces of the app do nothing, and #555 left all three doing it.**
  `InfoLabel` takes a `kind` whose CSS section is empty and a `title` it destructures away
  before rendering, so the two `MovieDetails` labels that pass a title have never shown
  one. `withCurrentPage` injects `useCurrentPage` into `UnmappedFilesTableConnector`, which
  takes no props and ignores it — the HOC's only consumer. And `polyfills.ts` guards
  `String.prototype.startsWith`, `endsWith` and the non-standard `contains`, none of which
  any browser that can run this app is missing, and whose last caller went away when #553
  rewrote `filterTypePredicates`. Each is a deletion or a fix rather than a conversion, so
  the conversion PR recorded them instead. `InfoLabel`'s title is the only one a user can
  see the absence of.

- **Two filter-builder defects survived #556 on purpose.** `getValue` reads a
  `numberFractionDigits` off the selected filter prop that no filter declares, so every
  number filter rounds a typed value to a whole number; the field is declared optional
  rather than removed, because a filter that wants decimals is the fix. And the tag list
  lookup in `FilterBuilderRowValue` can miss — a saved filter naming a quality profile,
  indexer or tag that has since been deleted — and renders the tag as the literal string
  "undefined" instead of the id it holds. Both are a line each, and neither belongs in a
  conversion PR.

- **Two table-options defects survived #557 on purpose.** The modal's page-size sync
  compared the previous `pageSize` prop against the current state instead of against the
  current prop, so a rejected value snaps the input back to the stored one the moment it
  loses focus — the error message stays, but the number the user typed does not.
  `NumberInput`'s focus guard is the only reason this is not obvious while typing. And
  `TableOptionsColumnDragSource` styled its wrapper with two class names its stylesheet has
  never declared; removing them is a no-op, but whatever the up/down affordance was meant
  to be was never built. The first is a one-word fix, the second a design call.

- **Two quality profile defects survived #558 on purpose, and they compound.** Clearing a
  custom format's score in the profile editor puts `null` on the wire, and the server
  answers 400 with `The JSON value could not be converted to System.Int32`. That body is
  ASP.NET's `ProblemDetails` object, not the array of validation failures every other 400
  from this API returns, so `getValidationFailures` reduces over an object and throws — the
  save fails *and* the modal cannot render why. The first is a coercion in
  `QualityProfileFormatItem` (or a `min` on the input); the second is a shape check in
  `Helpers/selectSettings.ts` that would cover every deserialisation failure, not just this
  one, and is worth doing on its own. Both reproduce on the pre-conversion build.
