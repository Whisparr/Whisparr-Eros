...
- **The `.js` conversion is finished: 4 files, 82 lines, and none of them can move.** Down
  from 114. The follow-on this thread asked for ran one cluster per PR: #551 deleted the 20
  ...
  #559 the config cleanup, #560 the jQuery tail. What is left is
  `Styles/Variables/{dimensions,fonts,animations,zIndexes}.js`, and it stays: `postcss.config.js`
  `require.resolve()`s those four paths and requires them through `require-nocache` to feed
  `postcss-simple-vars`, and Node cannot require `.ts`. Converting them means changing how
  the CSS build reads its variables — a build decision, not a conversion, and the only one
  left to make.

  Two pieces of config stay with them. **`.js` stays in webpack's `resolve.extensions`** for
  as long as those four do. **The `.js` block in `eslint.config.js` stays permanently**,
  whatever happens to `Styles/Variables`: it also lints `frontend/babel.config.js`,
