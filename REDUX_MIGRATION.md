...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **186** of 1,219 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **8,380** across 94 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **11** |
| Remaining `*Connector` files | 66 | **53** |
| Files touching React Query | 35 | **66** |
| zustand stores | 0 (not installed) | **installed, 24 files** |
...
| **Performer** *(hybrid)* — 20 redux files. Details, scenes tab, add flow, edit modal. **Index options done, #504; bulk editing and both delete paths done, #505; edit modal and the slice done, #506. 5 files left.** | ~~Index options → `performerIndexOptionsStore`~~; ~~filter modal `sectionItems`~~; ~~select footer~~; ~~tags modal~~; ~~both Delete modals~~; ~~edit modal~~; ~~`performerActions` (675 loc), `EditPerformerModalContentConnector.js`, `createAllPerformersSelector.ts`, `createPerformerClientSideCollectionItemsSelector.js`, `createPerformerSelector.js`, `PerformersAppState.ts`~~. Still to go: `performerScenesActions` (details), `addPerformerActions` (add flow) | — |
