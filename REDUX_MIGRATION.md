...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **19** of 1,156 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **2,090** across 37 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **1** |
| Remaining `*Connector` files | 66 | **9** |
...
**Next: section 11b** — download client options, the single-object form, then **11c** —
download clients, the provider list with a manage modal and the six files outside
`Settings/` that read or fetch the slice today. 11c takes `settings` to nothing:
`createProviderSettingsSelector`, `createSortedSectionSelector`,
`createClientSideCollectionSelector`, `withAdvancedSettings` and the last of the provider
