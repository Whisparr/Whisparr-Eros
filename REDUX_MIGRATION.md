...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **189** of 1,224 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **8,487** across 96 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **12** |
| Remaining `*Connector` files | 66 | **54** |
| Files touching React Query | 35 | **67** |
...
| **Performer** *(hybrid)* — 20 redux files. Details, scenes tab, add flow, edit modal. **Index options done, #504; bulk editing and both delete paths done, #505. 8 files left.** | ~~Index options → `performerIndexOptionsStore`~~; ~~filter modal `sectionItems`~~; ~~select footer~~; ~~tags modal~~; ~~both Delete modals~~; ~~`createAllPerformersSelector.ts`, `createPerformerClientSideCollectionItemsSelector.js`~~. `performerActions` 675 → 76 loc, keeping only `fetchPerformers`, `savePerformer` and the edit modal's `setPerformerValue`. Still to go: `performerScenesActions`, `addPerformerActions`, `EditPerformerModalContentConnector.js` | — |
