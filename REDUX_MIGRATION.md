...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **207** of 1,232 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **9,628** across 102 |
...
| **Studio** *(hybrid)* — 20 redux files. Same shape as Performer. **Index options done, #500; bulk editing done, #501. 8 files left, 1 of them gated on Phase E.** | ~~Index options → `studioIndexOptionsStore`~~; ~~filter modal `sectionItems`~~; ~~select footer~~; ~~both Delete modals~~; ~~`DeleteStudioModalConnector.js`, `createAllStudiosSelector.ts`~~. `studioActions` 509 → 123 loc, keeping only the edit modal's `setStudioValue`; `studioMoviesActions`, `studioScenesActions` | — |
