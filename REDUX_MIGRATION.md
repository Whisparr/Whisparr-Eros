...
- ~~Re-home the Sentry middleware — `createSentryMiddleware.js` is a redux middleware and
  ...
  Sentry is retired from the frontend outright: the middleware, `ErrorBoundary`'s
  `captureException`, both `@sentry/*` packages and the (already dead) piwik loader. The
  backend keeps `analytics` and `userHash` in `initialize.json`; nothing in the frontend
  reads them.
