...
- **53 `.js` files are still unchecked** — down from 114. The follow-on this thread asked
  for was scoped and is being run one cluster per PR: #551 deleted the 20 dead files, #552
  converted the `Utilities/` leaves. What is left, in the order it is queued: `Helpers/Props`
  plus `commandNames` and `dragTypes` (6 files, and the barrel behind ~446 import sites),
  `Styles/Themes` (3), the one-off components (17, mostly PropTypes → `Readonly<Props>`),
  the filter builder (9, two classes), table options (5, react-dnd), and the
