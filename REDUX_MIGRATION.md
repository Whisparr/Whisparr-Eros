...
- **7 `.js` files are left, and the mechanical work is done** — down from 114. The
  ...
  **`Styles/Variables/{dimensions,fonts,animations,zIndexes}.js` cannot be converted**:
  ...
  renders an `AppError`. `frontend/jsconfig.json` and the `.js` block in
  `eslint.config.js` can go now; `.js` stays in webpack's `resolve.extensions` for as long
  as the four `Styles/Variables` files do.
