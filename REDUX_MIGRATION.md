...
- **27 `.js` files are still unchecked** — down from 114. The follow-on this thread asked
  ...
  `dragTypes`, #554 the theme palettes, #555 the one-off components. What is left, in the
  order it is queued: the filter builder (8, two classes, one fewer than planned now that
  `tagShape` is typed), table options (5, react-dnd), and the `QualityProfile*`
  drag-and-drop group (7, the largest and the one where a typing mistake is most likely to
  change drag behaviour). Two things are deliberately not in that list.
