...
| --- | --- | --- |
| Files importing `react-redux` | 327 of 1,255 | **212** of 1,232 |
| Lines under `frontend/src/Store/` | 15,374 across 138 files | **9,714** across 103 |
| Redux slices registered in `Store/Actions/index.js` | 35 | **14** |
| Remaining `*Connector` files | 66 | **55** |
| Files touching React Query | 35 | **65** |
| zustand stores | 0 (not installed) | **installed, 20 files** |
...
| **Studio** *(hybrid)* — 20 redux files. Same shape as Performer. **Index options done, #500. 13 files left, 1 of them gated on Phase E.** | ~~Index options → `studioIndexOptionsStore`~~; ~~filter modal `sectionItems`~~. `studioActions` down to 509 → 196 loc, keeping only the editing half; `studioMoviesActions`, `studioScenesActions`, `DeleteStudioModalConnector.js` | — |
...
which belongs to the editing half.
