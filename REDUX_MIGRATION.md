...
- **14 `.js` files are still unchecked** — down from 114. The follow-on this thread asked
  for was scoped and is being run one cluster per PR: #551 deleted the 20 dead files, #552
  converted the `Utilities/` leaves, #553 the `Props` barrel with `commandNames` and
  `dragTypes`, #554 the theme palettes, #555 the one-off components, #556 the filter
  builder, #557 the table options modal. One mechanical cluster is left: the
  `QualityProfile*` drag-and-drop group (7 files, the largest and the one where a typing
  mistake is most likely to change drag behaviour). Two things are deliberately not in that
  list.
  **`Styles/Variables/{dimensions,fonts,animations,zIndexes}.js` cannot be converted**:
  ...
  renders an `AppError`. When the mechanical clusters are done, `frontend/jsconfig.json`
  and the `.js` block in `eslint.config.js` go, and `.js` comes out of webpack's
  `resolve.extensions` last.
