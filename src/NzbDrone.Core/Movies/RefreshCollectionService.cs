    ...
    public class RefreshCollectionService : IExecute<RefreshCollectionsCommand>
    {
        ...
        private void SyncCollectionMovies(MovieCollection collection)
        {
            if (collection.Monitored)
            {
                ...
                var excludedMovies = _importListExclusionService.GetAllByType(ImportExclusionType.Movie).Select(e => e.ForeignId);
                var moviesToAdd = collectionMovies.Where(m => !existingMovies.Contains(m.TmdbId)).Where(m => !excludedMovies.Contains(m.TmdbId.ToString())).ToList();
