    ...
    public class ApiKeyValidationCheck : HealthCheckBase
    {
        ...
        public override HealthCheck Check()
        {
            if (_configFileProvider.ApiKey.Length < MinimumLength)
            {
                ...
                return new HealthCheck(GetType(), HealthCheckResult.Warning, HealthCheckReason.MinimumApiKeyLength, _localizationService.GetLocalizedString("ApiKeyValidationHealthCheckMessage", new Dictionary<string, object> { { "length", MinimumLength } }), "#invalid-api-key");
            }
