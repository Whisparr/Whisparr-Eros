...
namespace NzbDrone.Core.Test.Configuration
{
    ...
    public class AllowedHostsParserFixture : TestBase
    {
        ...
        {
            var result = AllowedHostsParser.Parse("whisparr.local, 192.168.1.5; myhost");

            result.Should().BeEquivalentTo("whisparr.local", "192.168.1.5", "myhost");
        }
        ...
        [TestCase("whisparr.example.co.uk")]
        [TestCase("my-host")]
        ...
        [TestCase("")]
        [TestCase("  ")]
        [TestCase("http://whisparr.local")]
        [TestCase("whisparr.local:6969")]
        [TestCase("whisparr local")]
        [TestCase("whisparr.*")]
        [TestCase("*")]
        [TestCase("*.")]
