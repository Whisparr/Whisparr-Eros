...
namespace NzbDrone.Core.Test.MediaFiles.TorrentInfo
{
    ...
    public class TorrentFileInfoReaderFixture : CoreTest<TorrentFileInfoReader>
    {
        ...
        private static byte[] MakeMultiFileTorrent(params string[] fileNames)
        {
            ...
            builder.Append($"e4:name6:Droned12:piece lengthi16384e6:pieces20:{new string('a', 20)}ee");
        ...
        {
            var fileNames = Subject.GetFileNamesFromTorrentFile(MakeSingleFileTorrent("Droned.1998.1080p.WEB-DL-DRONE.mkv"));

            fileNames.Should().ContainSingle(f => f.EndsWith("Droned.1998.1080p.WEB-DL-DRONE.mkv"));
        }

        [Test]
        public void should_get_file_names_from_multi_file_torrent()
        {
            var fileNames = Subject.GetFileNamesFromTorrentFile(MakeMultiFileTorrent("Droned.1998.1080p.WEB-DL-DRONE.mkv", "Setup.exe"));
        ...
        {
            var hash = Subject.GetHashFromTorrentFile(MakeSingleFileTorrent("Droned.1998.1080p.WEB-DL-DRONE.mkv"));
