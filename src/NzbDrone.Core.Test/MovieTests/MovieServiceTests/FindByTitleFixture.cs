...
namespace NzbDrone.Core.Test.MovieTests.MovieServiceTests
{
    ...
    public class FindByTitleFixture : CoreTest<MovieService>
    {
        ...
        public void Setup()
        {
            ...
            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(2))))
                .Returns(otherCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(3))))
                .Returns(dualCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(4))))
                .Returns(differentCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(5))))
                .Returns(invalidCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(6))))
                .Returns(bellaCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(7))))
                .Returns(evilCredits);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(8))))
                .Returns(ariAlectra);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(9))))
                .Returns(amhyraShy);

            Mocker.GetMock<ICreditService>()
                .Setup(s => s.GetAllCreditsForMovieMetadata(It.Is<int>(s => s.Equals(10))))
                .Returns(cocoLovelock);
            ...
            var scenes = Builder<Movie>.CreateListOfSize(2000)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 5)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 3)
                                        .With(x => x.MovieMetadata.Value.Studio = studio)
                                        .TheNext(1)
                                        .With(x => x.Title = "White Winter")
                                        .With(x => x.MovieMetadata.Value.ReleaseDate = "2021-01-09")
                                        .With(x => x.MovieMetadata.Value.Id = 4)
                                        .With(x => x.MovieMetadata.Value.Studio = studio)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 6)
                                        .With(x => x.MovieMetadata.Value.Studio = studio)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 7)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 8)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 10)
                                        .TheNext(1)
                                        ...
                                        .With(x => x.MovieMetadata.Value.Id = 11)
